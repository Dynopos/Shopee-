...
class PreviewController extends Controller
{
    public function index()
    {
        ...
        if (!$draft) return redirect()->route('upload');
    ...
    public function uploadToShopee(Request $request, ShopeeService $shopee)
    {
        ...
        try {
            $draft      = session('listing_draft');
            $imageUrl   = $shopee->uploadImage($draft['image_path'], $account->access_token, $account->shop_id);
            ...
            $response   = $shopee->createProduct([
                ...
                'seller_stock'     => [['stock' => (int) $data['stock']]],
