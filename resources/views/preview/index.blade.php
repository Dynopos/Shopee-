...
@else
@php $d = session('listing_draft', []); @endphp
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-3">
                <img src="{{ $d['image_url'] }}" class="w-full rounded-xl object-contain max-h-56">
            </div>
            @endif
            <div class="card space-y-4">
                <div>
                    <label class="form-label">Harga (MYR)</label>
                    <input type="number" name="price" step="0.01" value="{{ old('price', $d['suggested_price'] ?? 0) }}" class="form-input">
                </div>
                <div>
                    <label class="form-label">Stok</label>
                    <input type="number" name="stock" value="{{ old('stock', $d['stock_quantity'] ?? 0) }}" class="form-input">
                </div>
                <div>
                    <label class="form-label">Kategori</label>
                    <input type="text" name="category" value="{{ old('category', $d['category'] ?? '') }}" class="form-input">
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-4">
            <div class="card">
                <label class="form-label">Tajuk Produk ({{ strlen($d['title'] ?? '') }}/120)</label>
                <input type="text" name="title" maxlength="120" value="{{ old('title', $d['title'] ?? '') }}" class="form-input">
            </div>
            <div class="card">
                <label class="form-label">Penerangan</label>
                <textarea name="description" rows="5" class="form-input resize-none">{{ old('description', $d['description'] ?? '') }}</textarea>
            </div>
            <div class="card">
                ...
                <input type="hidden" name="keywords" value="{{ json_encode($d['keywords'] ?? []) }}">
            </div>
            <div class="card">
                <label class="form-label">Keistimewaan Produk</label>
                <ul class="space-y-2 mt-2">
                    @foreach($d['highlights'] ?? [] as $i => $h)
                    <li class="flex items-center gap-2">
                        <span class="text-green-500 text-xs font-bold">✓</span>
                        <input type="text" name="highlights[]" value="{{ $h }}" class="flex-1 form-input">
                    </li>
                    @endforeach
                </ul>
            </div>
