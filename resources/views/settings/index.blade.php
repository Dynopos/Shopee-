    ...
    <div class="card">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center justify-between p-3.5 bg-gray-50 rounded-xl">
                <div><p class="text-sm font-semibold text-gray-900">{{ $a->shop_name }}</p><p class="text-xs text-gray-400">Shop ID: {{ $a->shop_id }}</p></div>
                <div class="text-right"><p class="text-xs text-gray-400">Token tamat</p><p class="text-xs font-semibold text-gray-600">{{ $a->token_expires_at->format('d/m/Y') }}</p></div>
            </div>
            @endforeach
        </div>
        @endif
